...
6. **Shooting** — tracers spawn at the muzzle and travel along **camera aim** (Policy A) with simple ballistic drop (`vel.y -= g·dt`). SMG drops more / slower; rifle flatter; sniper scope fastest/flattest. Hits flash range targets; **+pts hit markers** stay screen-fixed HTML overlays, while **standing range tags** (≈50m / silhouette lane, etc.) are world-space sprites that shrink with distance. Cheap Web Audio SFX (fire / hit / bullseye / miss) unlock on first gesture.
