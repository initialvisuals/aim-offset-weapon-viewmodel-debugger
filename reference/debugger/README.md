...
6. **Shooting kit** — **long tracer streaks** + ballistic drop; mags SMG 30 / DMR 20 / bolt sniper 5 (`mag/capacity · ∞`) — capacity follows the **weapon**, not the optic (a scope on the DMR keeps 20). Empty = dry-click. **SMG auto, in-line recoil:** **B** toggles **SEMI / AUTO** (SMG only; rifle/sniper stay semi). AUTO hold-LMB ~1200 rpm; SEMI is one shot per click. HUD shows fire mode next to ammo. **R** reload (~1.2s SMG / ~1.4s DMR / ~2s sniper) with viewmodel dip. Bolt sniper cycles ~0.65s between shots (handle flick). Live shots eject **shell casings** (FIFO default **80**, fade **12s** after they sleep; 0 = until cap recycles) and a short **muzzle flash**. Hits spawn **small ragged impact holes + sparks** (env/sil FIFO default **120**, fade **18s**; 0 = FIFO only). **Paper-target holes persist** until the bench **RESET TARGETS** button. Screen-fixed +pts markers. Cheap Web Audio SFX.
...
12. **Settings (`O`)** — pauses gameplay: game style, **hip reticle** toggle, aim/bore rays, **brightness / gamma** (CSS filter + mild fog lift; defaults **1.00 / 1.00**), **FX** (**hole cap** default **120**, 20–400; **casing cap** default **80**, 10–250; **hole fade** default **18s**, 0 = FIFO only, paper holes stay until table reset; **casing fade** default **12s** after sleep, 0 = until cap recycles; lowering a cap trims oldest immediately), **Lighting** (clock default **18:30** dusk plus ambient/fill/hemi/sun/rim/moon/ACES-exposure multipliers, 1.00 = authored ToD, **god rays** default **0.90**, **bloom** default **0.45**), **fog** enable + near/far (linear `THREE.Fog`, defaults **ON / 375 / 520**, softer/farther), **PLUGE** grey strip, zero distance, look sens, ADS look mul, **crouch height** 0–100%, controls cheat.
...
| Lighting | Settings | Clock **18:30** + intensity muls **1.00×** on ToD bases (ambient / fill / hemi / sun / rim / moon / ACES exposure) + **god rays** default **0.90** (0–2, 0 = off) + **bloom** default **0.45** (0–2, 0 = off). Display grade unchanged. |
| Show PLUGE strip | Settings | **OFF** — Black/Low/Mid/High/White overlay (unfiltered) |
| Hole cap / fade | Settings FX | Cap default **120** (20–400). Fade default **18s** after spawn (0 = FIFO only). Env/scuff + non-paper punches; paper holes wait for table reset. Berm popups still clear on hide. |
| Casing cap / fade | Settings FX | Cap default **80** (10–250). Fade default **12s** after sleep (0 = until cap recycles). Lowering a cap FIFO-trims extras immediately. |
