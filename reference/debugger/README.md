...
- Tracer policy: long visible streaks. **Sim** = spawn at muzzle with **ballistic zero** launch at `effectiveZeroDist()` (irons 100 m; other optics `zeroDist`; documents HoB). **Arcade** = direction = camera forward (reticle-faithful). Gravity per weapon/optic.
