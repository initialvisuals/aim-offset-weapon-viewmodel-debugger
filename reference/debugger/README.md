...
| Pass lab PIP | Settings | **OFF** — ~200 px center crop + 1 px red sample rect. Source: **final** (composite), **scene** (pre-bloom dest), **heat grab** (only fills when ground / height-fog heat haze is on; barrel cards do not grab). Label shows the resolved source (falls back if that RT is idle). |
